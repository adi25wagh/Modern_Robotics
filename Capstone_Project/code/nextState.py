...
class nextState:
    ...
    def wheelOdometry(self, current_state, theta_dot):
        # compute the twist generated by the body wrt the inputs to the wheels
        V_b = np.dot(self.H_mat, theta_dot*self.dt)
        wbz,vbx,vby = V_b
        phi = current_state[0]

        if wbz < 1E-3:
            # Assign zero if rotation along Z axis in near-zero
            dqb = np.array([0,vbx,vby])
        else:
            # compute derivative in deltaq for the {b} frame.
            dqb = np.array([wbz, 
                            vbx*np.sin(wbz)+vby*(np.cos(wbz)-1)/wbz,
                            vby*np.sin(wbz)+vbx*(1-np.cos(wbz))/wbz])
            
        # transformation matrix for conversion from {b} to {s}
        T_s = np.array([[1, 0, 0],
                     [0, np.cos(phi), -np.sin(phi)],
                     [0, np.sin(phi), np.cos(phi)]])
        
        # compute the position of the base.
    ...
    def updateRobotAngles(self, current_state, control_inputs):
        ...
        # Update robot joint angles and wheel angles
